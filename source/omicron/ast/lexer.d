...
Token parseCharacter(LexerState state) {
	auto location = state.location;
	Appender!string result;
	state.popFront();
	if (!state.empty) result.put(state.parseChar());
	if (state.empty || state.front != '\'') throw new LexerException("Unterminated string constant.", location);
	state.popFront();
	return Token(TokenType.ud_char, location, result.data);
}
